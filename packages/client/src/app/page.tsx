...
const LiveTranscription = () => {
  ...
  const connectToSSE = () => {
    ...
    const eventSource = new EventSource(
      "http://localhost:4000/api/screenpipe/transcription"
    );
  ...
  const displayText = finalText + partialText;
      ...
      const fetchAudio = async () => {
        try {
          const response = await fetch(
            "http://localhost:4000/api/whisper/audio-generation"
          );
          if (response.ok) {
    ...
    return (
      <div className="p-4">
        ...
        {isConnected ? (
          <button className="bg-amber-600" onClick={() => disconnectFromSSE()}>
            End Audio
          </button>
        ) : (
          <button className="bg-amber-600" onClick={() => connectToSSE()}>
            Start Audio
          </button>
        )}
